...
public class RepoController : ControllerBase
{
    ...
    public IActionResult List()
    {
        ...
        var repos = _registry.GetAll()
            .Select(r => new { id = r.Id, name = r.Name, handle = r.Handle, path = r.Path, exists = r.Exists, isGitRepo = r.IsGitRepo, isSelf = r.IsSelf, visibility = r.Visibility, provider = r.Provider, localPort = r.LocalPort, localApps = AppsJson(r.LocalApps) });
        return Ok(repos);
    }
    ...
    {
        _logger.CountRequest();
        if (!_registry.SetProvider(id, request?.Provider))
            return NotFound(new { error = "Unknown repository id." });
