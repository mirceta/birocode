...
public static class ArchPoliceman
{
    ...
    public static string Prompt(string? boardGoal)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("1. Call board_integrity. It is the harness's own verdict from the REAL facts (the assignee's clone, the PR on GitHub, the deploy log): which cards are dishonest (their column is ahead of reality), which assignees are stuck (pinged, no PR, blocked or silent past the window), which cards are manual or externally owned (not yours), and every card that already carries \"human assistance requested\" and who raised it.");
        sb.AppendLine("2. READ EVERY AGENT. Call list_tasks. For EVERY card in Doing, Committed or PR open that has an assignee and is neither manual nor externally owned, call read_transcript on that assignee (its handle from the card, tail 4) and judge from the agent's own words what is going on. Record it with observe_card(id, state, summary): state is one of working | waiting-review | asked-question | blocked | claims-done | idle | errored; summary is ONE sentence in plain words that stands alone, quoting what you read and when (\"asked for the production API key 30 min ago, no answer yet\"). Re-observe only when the state or the summary changed; clear_observation when the card is delivered or the agent is plainly back on track. A claimed repo refuses the read — say so, never guess.");
        ...
        sb.AppendLine("4. FLAG: flag_needs_human(id, reason) for an assignee that is genuinely stuck — it asked a question nobody answered, said it is blocked, or errored, and that has stood past the window — or a card that keeps claiming more than the facts show and nobody is fixing it; clear_needs_human(id) when a card YOU flagged is honest again. You cannot dispatch, edit, assign, delete or move a card by claim — those tools are refused for you — so name what the Operator or the arch should do instead. Leave manual cards alone. Leave EXTERNALLY OWNED cards (externalOwner set: a different human developer's, out of our domain) entirely alone — never read, observe, move, flag or report them as stuck or dishonest; they are not yours to judge. Never re-ping anyone.");
        sb.AppendLine("5. PROVENANCE. Everything you record — an observation, a flag, a move — carries your name, the time and this session's id; the Operator sees it on the card and finds the tool call in History. Write every summary and reason so it stands alone without this conversation.");
        sb.AppendLine("6. Answer with a short verdict: N honest · N dishonest · N need human · N manual · N external, then one line per card you moved (#ref — from → to — the PR), per card you observed as blocked, asked-question, errored or claims-done (#ref — what the agent said), and per flagged card (#ref — why — what should happen). If nothing changed since your last pass, say \"no change\" and stop.");
        sb.AppendLine();
