...
public class ArchMcpServer
{
    ...
    private ArchAgentService.ToolOutcome? Call(string name, JsonObject args)
    {
        ...
        return name switch
        {
            ...
            "start_arch_goal" => _arch.ToolStartArchGoal(S("goal"), S("repos"), S("tasks"), IN("maxIterations"), B("requireMerged") == true, S("machine")),
    ...
    public static JsonArray ToolsList() => new(
        ...
        Tool("start_arch_goal",
            "Open a GOAL CONVERSATION (openspec arch-goal-conversations) — ONLY when the Operator asked for it in this conversation, never on your own initiative. A new arch conversation is created that OWNS the named repo agents and board tasks for the goal's duration, a goal loop (drive, capped) is armed on it, and it alone is woken by what happens on those repos and tasks (turns, loop events, task status changes) — this Operator-facing conversation stays free and receives no repo wake-ups. Returns started with the goal id and the conversation; refused as owned when a repo or task already belongs to a running goal, unmanaged for a repo outside the arch scope, gate-closed when the autopilot gate is closed. The goal ends when its own verification passes AND every owned task is at least pr-opened (pr-merged with requireMerged) on the board; its summary is then posted here as one message.",
            Schema(("goal", "string", "what done looks like, in the Operator's words", true),
                ("repos", "string", "comma-separated repo agents the goal owns: handles (spacex/prg#2), repoIds or unique names from list_agents", false),
                ("tasks", "string", "comma-separated board task ids from list_tasks the goal owns (their assignees are owned too)", false),
                ("maxIterations", "integer", "the loop cap, 1–100 (default 20)", false),
                ("requireMerged", "string", "\"true\" when the goal counts as done only once the owned tasks are pr-merged (default: pr-opened)", false),
                ("machine", "string", "default machine for bare repo names (\"self\" or a machine label)", false))),
        Tool("list_arch_goals",
            "Every goal conversation: id, conversation (id + name), goal, state (running | done | stopped | capped | error), busy, the repos and board tasks it owns (with each task's status and assignee), iterations and cap, last wake, outcome, queued Operator messages — plus the legacy-broadcast setting and the arch inbox (managed repo events nobody owned). Read-only.",
            new JsonObject { ["type"] = "object", ["properties"] = new JsonObject(), ["additionalProperties"] = false }),
        Tool("stop_arch_goal",
            "Stop a running goal conversation on the Operator's ask: its loop stops, it releases the repos and tasks it owned and becomes available again; the summary is posted here. A goal that already ended is reported, not changed.",
