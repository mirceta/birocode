...
public class ArchMcpServer
{
    ...
    private ArchAgentService.ToolOutcome? Call(string name, JsonObject args)
    {
        ...
        return name switch
        {
            ...
            "update_task" => _arch.ToolUpdateTask(S("id"), S("status"), S("title"), S("note"), S("branch"), S("commit"), S("pr")),
    ...
    public static JsonArray ToolsList() => new(
        ...
        Tool("update_task",
            "Change a task's status, title or note, and relay an agent's closing-line claim. \"TASK COMMITTED <id> <branch> <commit>\" → status committed with branch+commit; \"TASK PR <id> <url>\" → status pr-opened with pr; \"TASK BLOCKED <id>: …\" → status todo with the reason in the note. The harness VERIFIES claims against the repo's git/PR state: a forward status lands no further than the verified state (the tool says so when it clamps — report that, the branch is not where the agent said). Backward moves are always allowed. Statuses from committed up are otherwise advanced by the harness itself when it observes the facts.",
