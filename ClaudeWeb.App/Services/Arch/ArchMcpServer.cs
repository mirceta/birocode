...
public class ArchMcpServer
{
    ...
    private ArchAgentService.ToolOutcome? Call(string name, JsonObject args)
    {
        ...
        return name switch
        {
            ...
            "hub_transfer" => _arch.ToolHubTransfer(S("action"), S("path"), S("from"), S("to"), B("overwrite") == true, S("jobId")),
    ...
    public static JsonArray ToolsList() => new(
        ...
        Tool("hub_transfer",
            ...
            Schema(("action", "string", "transfer (default) | status (one job by jobId, or every recent job)", false),
                ...
                ("path", "string", "the hub path (e.g. prg/fixtures/customers.json); required for a transfer", false),
