...
public class TasksMcpServer
{
    ...
    public static JsonArray ToolsList() => new(
        ...
        Tool("list_tasks",
            "The task graph: every task (id, title, note, repoId, status todo|doing|done) and every dependency edge (source depends on target). Call this before linking so you use real ids.",
            new JsonObject { ["type"] = "object", ["properties"] = new JsonObject(), ["additionalProperties"] = false }),
        Tool("create_task",
        ...
        Tool("update_task",
            "Edit a task: title, note, repoId, or status (todo | doing | done). Only the fields you pass change.",
            Schema(("id", "string", "the task id", true), ("title", "string", "new title", false), ("note", "string", "new note", false),
                ("repoId", "string", "new repository label (empty string clears it)", false), ("status", "string", "todo | doing | done", false))),
        Tool("link_tasks",
