...
await browser.close();
server.close();
console.log(JSON.stringify({ drive, lit, states, cards, movedShown: /was Doing/.test(moved) && /Asked a question/.test(moved), errs }));
process.exit(errs.length === 0 && drive.length === 9 && lit === 7 && states.length === 4 && cards === 3 ? 0 : 1);
