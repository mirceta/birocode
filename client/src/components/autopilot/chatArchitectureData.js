// Diagram specs for the "How chat works" dock tab. Each is a hand-placed concept
// graph rendered by <ChatGraph> (cytoscape). Every node carries role/desc/src so a
// click reveals what it is and which real file it maps to. Facts are drawn from the
// actual code (cited in each `src`).
//
// This is a FIRST-CLASS module of the app — ported from the old rolling
// understanding-app snapshot (client/public/chat-architecture/data.js), which was
// iframed. The dock tab is a permanent product surface, so its diagram source lives
// here in the React tree and is maintained like any other code, not vendored.
...
//
// OVERLAY ANNOTATIONS (the §2/§3 prose, folded into this one map):
//   • bind: 'convo'|'run'|'session'  — the three grains of the conversation↔repo↔run
//     binding; bindPath:true marks a pure path connector (lit, but not grain-coloured).
//     Drives the "Show the binding" highlight.
//   • fate: 'wiped'|'survives' (+ twist:true) — what a browser refresh destroys vs.
//     keeps. Drives the "⟳ Refresh!" overlay. twist = the one box whose survival the
//     spatial layout does NOT explain for free (localStorage sits on the device yet
//     survives, because it holds the sessionId).
// Nodes without these keys are dimmed while an overlay is active.
export const SYSTEM_MAP = {
  nodes: [
    ...
    { id: 'ctx', label: 'ChatContext.jsx', x: 120, y: 110, grp: 'client', p: 'T_device',
      bind: 'convo', fate: 'wiped',
      role: 'Client state (in your browser, RAM)',
      desc: 'The CONVERSATION grain: holds convos (messages), seqRefs (last seq seen per chat) and abortRefs (the live SSE reader). A browser refresh WIPES all of it — watermarks reset to 0. On mount / tab-focus it reconciles and re-attaches to the running Run via stream?after=N, deduping by seq.',
      src: 'client/src/context/ChatContext.jsx' },
    { id: 'ls', label: 'localStorage', x: 120, y: 250, grp: 'client', kind: 'db', p: 'T_device',
      bind: 'session', fate: 'survives', twist: true,
      role: 'Tiny persistent store on the device',
      desc: 'The SESSION grain, on the device side: remembers the active sessionId per chat so a reload can re-attach to the same backend Run. THE TWIST — it sits inside the device box yet SURVIVES a refresh (everything else in this box is wiped). That single exception is the part spatial layout cannot explain for free.' },
    ...
    { id: 'runsvc', label: 'RunSessionService.cs', x: 430, y: 170, grp: 'backend', p: 'B_runs',
      bind: 'run', fate: 'survives',
      role: 'Owner of every chat Run',
      desc: 'The RUN grain: TryBeginRun(repoId, lane) hands out ONE writer slot per (repo · lane). Keeps a seq-numbered event buffer (seq is monotonic per repo ACROSS runs) and replays it to every subscriber, so attach/re-attach never drops or doubles an event. SURVIVES a refresh — the run keeps going, every event stays buffered with its seq.',
    ...
    { id: 'builder', label: 'builder slot', x: 360, y: 295, grp: 'backend', kind: 'slot', p: 'B_runs',
      ...
      desc: 'One per repo. Every turn that WRITES must claim it first. You, the classifier and loop mode all call the same TryBeginRun — so they can never run concurrently on one agent. First caller wins. Part of the RUN grain; survives a refresh.' },
    ...
    { id: 'claude', label: 'claude process', x: 960, y: 20, grp: 'cli', kind: 'proc', p: 'T_cli',
      bindPath: true, fate: 'survives',
      role: 'The actual Claude Code CLI',
      desc: "Runs on the RunSession's own Cts token — never the HTTP request's RequestAborted. That is why a disconnected phone does not kill the turn, and why it SURVIVES a refresh. Emits stream-json on stdout." },
    { id: 'jsonl', label: '~/.claude/…/<sid>.jsonl', x: 960, y: 250, grp: 'store', kind: 'db', p: 'T_disk',
      bind: 'session', fate: 'survives',
      role: 'Append-only transcript on disk',
      desc: "The SESSION grain's durable identity: the CLI writes the full conversation here regardless of who is watching, so it SURVIVES everything. It is the autopilot's read source — the tick reads the last reply from this file to decide what (if anything) to send next." },
  ],
  edges: [
    ...
    { s: 'runsvc', t: 'ctx', label: 'SSE events · seq-dedup', rel: 'stream', wipe: true },
