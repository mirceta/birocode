...
// Presentational scoreboard (openspec fleet-status-panels): the stats + the three
// hand-rolled SVG views, given a ready `data` payload and a window selector. Split
// out of Scoreboard.jsx so BOTH the harness's own header Scoreboard (which fetches
// /analytics and wraps this in a collapsible bar) and the Management Fleet Status
// Scoreboard tab (which fetches a PEER's analytics on demand) render the identical
// look from the same code. This component never fetches and never collapses — its
// callers own data, window state, and any surrounding chrome.
...
export { fmtDur, fmtClock, fmtCost, WINDOWS };

// The window tab strip — rendered by callers that let the viewer switch windows.
export function ScoreboardWindows({ window, onWindow, t }) {
  return (
...
export default function ScoreboardView({ data, window, t }) {
  if (!data) return null;
...
// Hero chart — a step-area of how many agents ran simultaneously across the window.
