...
public class ArchAgentTests : IDisposable
{
    ...
    [Fact]
    public void Mcp_tools_list_names_the_nineteen_tools_with_required_args()
    {
        var tools = ArchMcpServer.ToolsList();
        var names = tools.Select(t => t!["name"]!.GetValue<string>()).ToList();
        Assert.Equal(new[] { "list_agents", "list_machines", "git_state", "read_transcript", "send_task", "upgrade_peer", "list_loops", "start_loop", "update_loop", "stop_loop", "list_tasks", "create_task", "update_task", "assign_task", "dispatch_task", "list_ideas", "idea_to_task", "remember", "recall" }, names);
    ...
    public void Mcp_catalogue_offers_machine_on_every_repo_addressed_tool()
    {
        ...
        Assert.Equal("<!-- arch-role v5 -->", ArchAgentService.RoleVersionMarker); // v5: loops on repo agents (openspec arch-loop-tools)
