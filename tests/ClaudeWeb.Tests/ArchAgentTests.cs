...
public class ArchAgentTests : IDisposable
{
    ...
    [Fact]
    public void Mcp_tools_list_names_the_twenty_nine_tools_with_required_args()
    {
        ...
        Assert.Equal(new[] { "list_agents", "list_machines", "git_state", "read_transcript", "send_task", "adopt_branch", "upgrade_peer", "list_loops", "start_loop", "update_loop", "stop_loop", "start_arch_goal", "list_arch_goals", "stop_arch_goal", "list_tasks", "create_task", "update_task", "assign_task", "add_leg", "remove_leg", "set_leg_role", "dispatch_task", "delete_task", "list_ideas", "idea_to_task", "remember", "recall", "hub_files", "hub_transfer" }, names); // + hub_files / hub_transfer (openspec hub-file-system); the board's checking tools left with the policeman conversation (openspec one-policeman)
