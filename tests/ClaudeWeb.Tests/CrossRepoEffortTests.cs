...
public sealed class CrossRepoEffortTests : IDisposable
{
    ...
    public void A_repo_agent_can_inspect_its_effort_and_a_driver_can_report_the_leg_it_drives()
    {
        ...
        Assert.Equal(new[] { "my_effort", "report_leg", "harness_help", "stash_prompt", "arm_my_loop", "hub_upload", "hub_download", "hub_files", "my_local_apps" }, (listed.Body!["result"]!["tools"] as JsonArray)!.Select(t => t!["name"]!.GetValue<string>()));   // + the self-service three (openspec repo-agent-harness-tools)
