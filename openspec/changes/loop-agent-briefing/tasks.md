...
- [x] 1.1 `BriefingRulesStore` (new, beside `LoopRecipeStore`): `briefing.json` via
...
- [x] 1.2 `LoopConfigStore`: fixed-frame consts (header, situational statement,
...
- [x] 1.3 `AutopilotService`: wrap the proposed text with the briefing in the drive
      ...
      untouched (D1, D3).
- [x] 1.4 Record the briefed marker + revision: `briefed: true` + `briefingRev` on the
      synthetic `user` event, on audit entries, and on the queue sent-history
      projection (D3).

## 2. API + dock UI: the always-visible Briefing editor

- [x] 2.1 `AutopilotController`: `GET /api/autopilot/briefing` (rules + rev + frame
...
- [x] 2.2 Dock **Briefing** section beside the loop section on each dock card:
...
- [x] 2.3 Gated loop detail + arm preview expose the live briefing composition so the
...
- [x] 3.1 `docs/loop-driven-agent-convention.md`: add the "How to behave" posture
...
- [x] 3.2 Honesty pass: grep docs/, understanding-app/, and code comments for the old
...
- [x] 4.1 Stub-CLI-simulator e2e on an isolated port: queue item, queue verify, goal
...
- [x] 4.2 Playwright on the isolated port: dock Briefing section visible beside the
