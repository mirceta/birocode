...
### Requirement: In-repo pilot consumer app

The system SHALL include, in this repository, a build-less, self-contained local app
that consumes the harness event feed and demonstrates that events are received. The app
SHALL poll the read-only feed endpoint, advance its watermark, and render the received
events. The app SHALL render events generically from the envelope (showing each event's
`type`, `source`, time, and payload) rather than being hardcoded to a single event
type, so that it also serves as the test bed for future event types. The app SHALL be
served through the harness's existing local-app mechanism and follow the local-exposure
rules (relative URLs, served under the harness proxy path).

#### Scenario: The pilot app shows turn-ended events arriving

- **WHEN** the pilot consumer app is open and an agent turn ends
- **THEN** on its next poll the app displays the corresponding `turn.ended` event with its repository, session, and status

#### Scenario: The pilot app renders an unknown event type generically

- **WHEN** an event whose `type` the app does not specifically know about appears in the feed
- **THEN** the app still displays it using the common envelope fields rather than ignoring it or breaking
