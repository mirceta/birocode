...
- [x] 1.1 `AutopilotService.SendPrompt`: the synthetic `user` event's `text`
...
- [x] 1.2 Confirm no other consumer reads the `user` event's `briefed` field
...
- [x] 2.1 `ChatContext.jsx`: `user` event handler passes `evt.text` only;
...
- [x] 2.2 `MessageBubble.jsx`: remove the `briefed` prop and the `msg__briefed`
      span; update the header comment. `Chat.jsx`: stop passing `briefed`.
- [x] 2.3 Remove the `.msg__briefed` rule from `chat.css`.
- [x] 2.4 i18n: delete `chat.briefedTag` and `chat.briefedTitle` from `en.json`
...
- [x] 3.4 `DockLoopControl.jsx`: "Queue audit" affordance in the queue section
...
- [x] 3.5 Ungated surfaces unchanged: dashboard/console audit slices keep
...
- [x] 4.1 Grep code comments, `docs/`, and `understanding-app/` for claims that
...
- [x] 4.2 Sweep loop-eval scenarios and any Playwright/e2e assertions for
      expectations on the raw-text bubble or the briefed tag; update them to
      expect the composed text.

## 5. Verify

- [x] 5.1 `npm --prefix client run build` and `dotnet build` (isolated dir per
      self-dev rules) pass.
- [x] 5.2 On an isolated :5200 instance with the stub CLI: arm a drive-mode
      ...
      composition.
- [x] 5.3 Same instance: reload the conversation from the transcript and assert
      the reloaded user turn matches the live bubble byte for byte (headless
      Playwright per docs/claude-web/browser-testing.md).
- [x] 5.4 Queue sent-history still shows the raw item text with the briefed
      badge; ungated audit rows unchanged.
- [x] 5.5 Same instance: after the item + verify sends, assert the ledger's two
      ...
      refuses.
- [x] 5.6 `openspec validate queue-loop-prompt-transparency --strict` passes.
