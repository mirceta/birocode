...
Makes the armed suggestion loop observably alive: suggest mode always pends
the classifier's best candidate, decisions are visible at the dock, loops on
missing repos resolve instead of being silently skipped, deny-list matching
is word-scoped, and (slice 2) a Claude-CLI classifier replaces the stub
scorer behind the same verdict contract.
