...
- [x] 1.1 `HostTimeController` with `GET /api/host-time` returning
...
- [x] 2.1 `HostClock.jsx` in `client/src/components/dashboard/`: skew-based 1 s
      ticker + 5 s resync, host-offset formatting (never phone-local), stale
      marker after ~3 failed polls
- [x] 2.2 Render in `dash__scoreboard-row` in `Dashboard.jsx`, gated by new
      `hostClock: 'advanced'` capability in `UiModeContext.jsx`
- [x] 2.3 `hostClock.css` matching the row's chip idiom + i18n keys in
      `en.json` / `tr.json`

## 3. Verify

- [x] 3.1 Build frontend + backend; Playwright on an isolated preview port:
