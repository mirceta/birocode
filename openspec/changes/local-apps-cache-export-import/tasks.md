...
- [ ] 1.1 Add an export builder in `DiscoverAppsPanel.jsx` (or a small helper
...
- [ ] 2.1 Add an `📤 Export` header button next to Load cache / Check / Import
...
- [ ] 2.2 Render the export section: read-only, capped-height scrollable
...
- [ ] 2.3 On `copyText()` returning false, select the textarea content and show
      a "copy manually (Ctrl+C)" hint instead of an error state.
- [ ] 2.4 Add i18n keys to `client/src/i18n/en.json` and `tr.json`; add any
      needed `.phone__discover*` styles in `client/src/pages/dashboard.css`.

## 3. Round-trip guarantee

- [ ] 3.1 Add a backend test in `tests/ClaudeWeb.Tests` that feeds a
...
- [ ] 4.1 `npm --prefix client run build` and `dotnet test` pass.
- [ ] 4.2 Headless Playwright check per `docs/claude-web/browser-testing.md`:
