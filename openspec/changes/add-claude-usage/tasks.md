...
- [ ] 1.1 Create `ClaudeWeb.App/Services/Accounts/ClaudeUsageService.cs`:
...
- [ ] 1.2 Add caching to the service: 5-minute TTL, single-flight refresh
...
- [ ] 1.3 Register the service and add `GET /api/claude-usage` in
...
- [ ] 1.4 Verify by hand on this machine: authenticated → real percentages
...
- [ ] 2.1 Extend the Claude chip in
      ...
      state untouched
- [ ] 2.2 Style in `accountChips.css`: compact meter row (fits the chip
...
- [ ] 2.3 Add i18n keys (`en.json`, `tr.json`): usage row labels ("5h",
      "Week"), "resets {time}", "usage unavailable", stale hint
- [ ] 2.4 Degradation behavior: `available:false` or fetch error renders the
...
- [ ] 3.1 Headless browser check (docs/claude-web/browser-testing.md):
      ...
      the upstream)
- [ ] 3.2 Grep gate: no token value can reach logs or responses — audit that
      `accessToken` is only referenced inside the probe method and never
      interpolated into strings
- [ ] 3.3 `openspec validate add-claude-usage --strict` passes; update
