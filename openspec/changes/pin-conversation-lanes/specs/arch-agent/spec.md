...
every lane. The title row and the lane strip SHALL stay pinned at the top of the
conversation column however far the transcript is scrolled, in every host of the
surface (studio tab, dashboard pop-up, Management App pane) and at every pane width:
only the message list scrolls beneath them and the composer stays at the bottom, with
no second scrollbar on the pane or the page. The same holds for every management-agent
conversation built on this column (the Tasks agent's surface).
...
#### Scenario: The lane strip survives a long conversation
- **WHEN** a conversation of sixty messages is scrolled to its end in a Management App pane narrower than 900px (two panes side by side on a laptop)
- **THEN** the Chat / Tools / History strip is still at the top of the pane and a click on Tools where it sits opens that lane, the composer is still at the bottom, and only the message list has scrolled
