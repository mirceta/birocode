...
- [ ] 1.1 Create `client/src/components/header/HeaderStatusStrip.jsx`: strip-level
...
- [ ] 1.2 Create `client/src/components/header/headerStrip.css`: full-width bar under
      the header + `.header-strip__row` (flex, wrap, gap 12px, Scoreboard
      `flex: 1 1 320px`), collapsed slim-bar styling
- [ ] 1.3 Add `headerStatusStrip: 'advanced'` to the FEATURES map in
      `client/src/context/UiModeContext.jsx` and gate the strip with it
- [ ] 1.4 Add i18n keys (`headerStrip.title` / `expand` / `collapse` / summary) to
      `client/src/i18n/en.json` and `tr.json`

## 2. Mount + remove from dashboard

- [ ] 2.1 Mount `<HeaderStatusStrip />` in `StudioShell` (`client/src/layout/Layout.jsx`)
...
- [ ] 2.2 Remove the `.dash__scoreboard-row` block and its `accountChips`/`hostClock`
...
- [ ] 3.1 `npm --prefix client run build`; run the harness from an isolated build on a
...
- [ ] 3.2 Confirm no polling while collapsed (no `/api/analytics`, account, usage or
      host-time requests from the strip) and polling starts on expand
- [ ] 3.3 Update any Playwright scripts that assert the scoreboard row inside the
