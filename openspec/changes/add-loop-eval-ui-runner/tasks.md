...
- [ ] 1.1 `tests/loop-eval/lib.mjs`: accept `LOOPEVAL_LIVE_TOKEN` — install it as
...
- [ ] 1.2 Fail fast on an invalid/revoked token: first authorized preflight call
      returning 401 produces a verdict naming the credential (mirror the
      existing wrong-password verdict copy)
- [ ] 1.3 Update `tests/loop-eval/README.md`: `LOOPEVAL_LIVE_TOKEN` row in the
...
- [ ] 2.1 `LoopEvalRunnerService` (singleton, own module extension per
...
- [ ] 2.2 Session minting: create a tagged (`loopeval-runner`) session directly
...
- [ ] 2.3 Spawn `node tests/loop-eval/<scenario>.mjs --live` with stdout/stderr
...
- [ ] 2.4 `LoopEvalController`: `POST /api/loopeval/runs` (409 with active-run
...
- [ ] 4.3 Isolated regression: `node tests/loop-eval/run-all.mjs` (default mode)
      still green — suite changes are additive
- [ ] 4.4 Browser test (per `docs/claude-web/browser-testing.md`): Tests tab
...
- [ ] 4.6 `openspec validate --strict` green
