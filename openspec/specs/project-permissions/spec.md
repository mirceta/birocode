...
## Purpose
Let the Operator assign a per-project permission preset (Read-only / Edit-only / Standard / Full)
from the desktop app, enforce it on every chat `claude -p` call for that project, and reflect it
read-only on the web Dashboard — so each project's agent is constrained to exactly the scope the
Operator approved, safe-by-default for unconfigured projects.
