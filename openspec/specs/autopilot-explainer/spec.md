...
agent turns and minutes, and how to launch it from the CLI; never CI) —
...
- **THEN** it describes the committed loop-eval suite — its two scenarios, the real-token cost, and the CLI launch command — rather than describing the layer as untracked scratch scripts
