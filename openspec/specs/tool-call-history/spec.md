...
Gives the End User a consolidated, durable record of the tool calls the agent made in
the currently active conversation — Bash, Read, Edit, Grep, and the rest — so they can
audit and follow what the agent actually *did*, independently of the message prose. The
live message stream scatters tool steps inline per turn and the message transcript
endpoint strips them, so this capability is the one surface that answers "what has this
agent done?" and survives a reload.
