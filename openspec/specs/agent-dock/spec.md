...
visible on the dashboard grid (`dashboard !== false`) SHALL render under **every**
filter state regardless of its branch classification — filters only ever narrow the
tabs of docks hidden from the grid, so the strip always shows at least every
grid-visible dock. Classification SHALL reuse the same
...
whose dock is hidden from the grid SHALL
...
- **THEN** the strip renders every grid-visible dock's tab plus the hidden docks' tabs whose repo's branch is `main` or `master`, and the dashboard grid keeps rendering exactly the docks it rendered before

#### Scenario: Only feature-branch agents

- **WHEN** the operator selects the **not on main** filter state
- **THEN** the strip renders every grid-visible dock's tab plus the hidden docks' tabs whose repo's branch is known and is neither `main` nor `master`
...
- **THEN** the dock's tab still renders in the strip, and hiding the dock (via its tab or the Agents page) makes it subject to the active filter like any other hidden dock

#### Scenario: Unknown branch shows only in All

- **WHEN** a dock hidden from the grid has no known branch and the filter is in a non-All state
...
- **THEN** the affected hidden docks' tabs enter or leave the filtered strip accordingly, without a page reload
...
conditions the tab's dot renders from — a hidden dock's tab SHALL match **running** if
and only if its dot currently shows the running state or the `!` marker — with no
additional polling and no new server state; grid-visible docks' tabs render under
**running** via the grid-visible exemption regardless of their dot. The status state
SHALL carry the same guarantees as the branch states:
...
- **THEN** the strip renders every grid-visible dock's tab plus the hidden docks' tabs whose dot currently shows the running state or the `!` unseen-result marker, and the dashboard grid keeps rendering exactly the docks it rendered before
...
- **WHEN** the **running** state is active and a dock hidden from the grid is idle with no unseen-result latch
