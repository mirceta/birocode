...
The chat composer's saved-prompts pop-up (the ⚙ modal) lets the End User keep reusable
prompting material alongside the composer: one-off **Prompts**, ordered **Plans**, and a
freeform **Notes** canvas. This baseline covers the Notes tab; Prompts and Plans are
seeded here as they are next touched (seed-and-grow).
